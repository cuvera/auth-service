...
async function fetchEmployeeDetails(email: string): Promise<string | null> {
    console.log('URL:', `${process.env.INTEGRATION_SERVICE_URL}/ingestion-service/api/v1/employees/email/${email}`);
    try {
        const response = await axios.get(
            `${process.env.INTEGRATION_SERVICE_URL}/ingestion-service/api/v1/employees/email/${email}`,
            {
                headers: {
